...
export default function Post() {
    ...
    const deletePost=()=>{
        bucketService.deletePost(post.$id).then((status)=>{
            if(status){
    ...
    <div className="py-8">
        <Container>
            <div className="w-full flex justify-center mb-4 relative border rounded-xl p-2">
                <img src={bucketService.getFilePreview(post.featuredImage)} alt={post.title} className="rounded-xl" />
                {isAuthor && (
                    <div className="absolute top-6 right-6">
                        <Link to={`/edit-post/${post.$id}`}>
                           <Button bg-color="bg-gray-500" className="mr-3">Edit</Button>
                        </Link>
